function [depthEst] = sphere_main_fg(src,lambda,noiseRad,noiseSrc,nSFSiter,nDepthIter)
%generates a sphere
...
%transforming params
[f,g] = paramSwitch(dx,dy,'pq');

pBoundary = f.*boundary;        %pBoundary variable contains f values
qBoundary = g.*boundary;        
...
for l = 2:nSFSiter-1
    for i = 2:imgSize-1
        for j = 1:imgSize
            if(roiRad(i,j))
            radx = 0; rady =0;
            radUp=16*(src(1)*f(i,j)+src(2)*g(i,j))+(4-f(i,j)^2-g(i,j)^2)*(4-src(1)^2-src(2)^2);
            radDown = (4 + f(i,j)^2+g(i,j)^2)*(src(1)^2+src(2)^2+4);
            temp_rad = radUp/radDown;
            radx=(-1*(16*src(1)-2*f(i,j)*(4-src(1)^2-src(2)^2)*radDown)-radUp*(2*f(i,j)*(4+src(1)^2+src(2)^2)))/radDown^2;
            rady=(-1*(16*src(2)-2*g(i,j)*(4-src(1)^2-src(2)^2)*radDown)-radUp*(2*g(i,j)*(4+src(1)^2+src(2)^2)))/radDown^2;
...
%transforming back to p,q
[p_est,q_est]=paramSwitch(p_est,q_est,'fg');
